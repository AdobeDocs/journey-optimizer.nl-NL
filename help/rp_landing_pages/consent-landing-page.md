---
solution: Journey Optimizer
product: Journey Optimizer
title: Toestemming beheren
description: Toestemming beheren
redpen-status: CREATED_||_2025-08-11_21-19-40
exl-id: ea8d895d-ec7e-40d9-814d-32135bcb8a2e
source-git-commit: 531e6f77af1d2a0f4ddf4f208e13feaa57326dee
workflow-type: tm+mt
source-wordcount: '258'
ht-degree: 3%

---

# Toestemming beheren{#section-overview}

Het beheren van toestemming is essentieel voor het opbouwen van vertrouwen met uw publiek en het waarborgen van naleving van privacyregels zoals GDPR. De toestemming in Adobe Journey Optimizer werkt naast [ privacybeheer ](../using/privacy/get-started-privacy.md) - met inbegrip van [ gegevensverzoeken ](../using/privacy/requests.md) en [ controlelogboeken ](../using/privacy/audit-logs.md) - en [ het beleid van het Beheer van Gegevens ](../using/action/action-privacy.md) dat gevoelige gebieden tegen wordt uitgevoerd naar derdesystemen beschermt.

U kunt toestemming op drie manieren behandelen: **opt-out voorkeur** voor abonnementen over e-mail, SMS, en duw; **toestemmingsbeleid** was van toepassing op uw campagnes en reizen om gegevensgebruik en marketing acties te controleren; en **voorkeurenbeheer** aan doelklanten die op de abonnementonderwerpen en de kanalen worden gebaseerd zij kiezen. De onderwerpen hieronder begeleiden u door elke stap.

## Goedkeuring beheren

:::: landing-cards-container
:::
![icon]( https://cdn.experienceleague.adobe.com/icons/shield-halved.svg)

Uitschakelen en Personalization-toestemming beheren

Leer hoe u de voorkeuren voor opt-out beheert en toestemming voor personalisatie voor berichten en campagnes afdwingt, terwijl u ervoor zorgt dat de privacywetten worden nageleefd.

[Leer hoe u toestemming beheert](../using/privacy/opt-out.md)
:::

:::
![icon]( https://cdn.experienceleague.adobe.com/icons/gear.svg)

Werken met beleid voor goedkeuring

Begrijp hoe te om toestemmingsbeleid tot stand te brengen en toe te passen om gegevensgebruik en marketing acties in overeenstemming met wettelijke en organisatorische regels te beheren.

[Beleid voor toestemming verkennen](../using/action/consent.md)
:::

:::
![icon]( https://cdn.experienceleague.adobe.com/icons/bullseye.svg)

Voorkeuren van uw klanten beheren

Leer hoe u abonnementsonderwerpen en kanaalvoorkeuren kunt beheren met behulp van het beleid voor toestemming, zodat u zich richt op klanten op basis van hun keuze en hun toestemming respecteert.

[De voorkeuren van uw klanten beheren](../using/action/preference-center.md)
:::

::::

## Aanvullende bronnen

- **[worden begonnen met privacy](../using/privacy/get-started-privacy.md)** - Overzicht van privacybeheer in Journey Optimizer, met inbegrip van gegevensverzoeken en controlelogboeken.
- **[Beheer van Gegevens](../using/action/action-privacy.md)** - bescherm gevoelige gebieden tegen wordt uitgevoerd naar derdesystemen door douaneacties.
- **[beheer e-mailopt-out](../using/email/email-opt-out.md)** - vorm e-mail-specifieke opt-out en unsubscription behandeling.
- **[beheer de opt-out van het tekstbericht](../using/sms/sms-opt-out.md)** - vorm SMS/MMS/RCS opt-out en unsubscription behandeling.
