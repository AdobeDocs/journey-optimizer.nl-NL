...
* **In-canvas bevestigingsalarm**: Wanneer het bouwen van reizen en campagnes, gebruik de **Alarm** knoop in het canvas om configuratiefouten te identificeren en op te lossen alvorens te publiceren. Leer hoe te om uw reizen [ problemen op te lossen en uw campagnes te herzien: ](../building-journeys/troubleshooting.md) campagnes van de Actie [ | ](../campaigns/review-activate-campaign.md) API-teweeggebrachte campagnes [ | ](../campaigns/review-activate-api-triggered-campaign.md) Geordende campagnes [.](../orchestrated/start-monitor-campaigns.md)
...
* [ de vernieuwing van het domeincertificaten van AJO niet succesvol ](#alert-certificates-renewal-unsuccessful) - waarschuwt wanneer het Middel of het Volgen certificaten op een subdomain van de douanedelegatie binnenkort verlopen of zijn verlopen
...
Als u een dergelijke waarschuwing ontvangt, worden de resolutiestappen hieronder vermeld.
...
### Verlenging van AJO-domeincertificaten is mislukt {#alert-certificates-renewal-unsuccessful}

>[!IMPORTANT]
>
>Dit alarm is slechts op kanaalconfiguraties van toepassing gebruikend het [ type van de douanesubdomain ](../configuration/delegate-custom-subdomain.md) delegatie.

Deze waarschuwing brengt u op de hoogte wanneer een Middel of het Volgen domeincertificaat op een subdomain van de douanedelegatie binnen 30 dagen verloopt of reeds is verlopen. Zonder geldige certificaten kan de e-maillevering en het bijhouden van koppelingen worden verstoord.

>[!NOTE]
>
>De controle stelt **wekelijks** in werking.

Als deze waarschuwing wordt geactiveerd, volgt u de onderstaande stappen om het probleem te onderzoeken en op te lossen.

1. Klik het alarm om beïnvloede [ subdomain ](../configuration/delegate-subdomain.md) in [!DNL Journey Optimizer] te openen.

1. Bekijk de details om te zien of certificaatvernieuwing nodig is.

   * Als de vervaldatum in de toekomst is, plan herstel-waakzaam kan tot 30 dagen van waarschuwing verstrekken.
   * Als het certificaat al is verlopen, voert u onmiddellijk actie.
   * Als het probleem niet is opgelost, wordt dezelfde waarschuwing de volgende week opnieuw geactiveerd.

1. In uw DNS het ontvangen oplossing, verifieer dat alle verslagen die voor de subdomeindelegatie worden vereist nog de waarden aanpassen die in [!DNL Journey Optimizer] worden getoond, met inbegrip van verslagen die voor SSL bevestiging worden gebruikt.
...
* [ test en publiceer reizen ](../building-journeys/publish-journey.md) - bevestig reisconfiguratie alvorens te publiceren
