...
* `_experience.journeyOrchestration.stepEvents.inDryRun` retourneert `true` wanneer de rit in de droge uitvoeringsmodus wordt uitgevoerd en `null` voor test- of live ritten (niet-droge run).
* `_experience.journeyOrchestration.stepEvents.dryRunID` retourneert de id van de droge runtime-instantie in de modus Droog. Voor test- of live ritten is dit `null` .
...
Wanneer het analyseren van **reis meldend metriek** gebruikend [!DNL Adobe Experience Platform] de dienst van de Vraag, moeten de droge looppas-geproduceerde stapgebeurtenissen worden uitgesloten. Hiervoor sluit u stapgebeurtenissen uit waarbij `inDryRun` `true` is (dus alleen gebeurtenissen opnemen waarbij `inDryRun` `null` of `false` is).
