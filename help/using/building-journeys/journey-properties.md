...
description: Leer hoe te om eigenschappen van uw reis met  [!DNL Adobe Journey Optimizer] te plaatsen
...
>abstract="Configureer algemene instellingen voor deze reis, zoals naam, codes, toegangsregels, tijdzone, datums, time-out en conflictbeheer. Alleen-lezen parameters zijn standaard verborgen. Welke opties beschikbaar zijn, is afhankelijk van de reisstatus, uw machtigingen en de productconfiguratie."
...
* Wijs [!DNL Adobe Experience Platform] Verenigde Markeringen aan uw reis toe, om hen gemakkelijk te classificeren en onderzoek van de campagnemelijst te verbeteren. [&#x200B; leer hoe te met markeringen &#x200B;](../start/search-filter-categorize.md#tags) werken
...
De **technische details van het Exemplaar** optie staat u toe om technische informatie over de reis te kopiëren die het steunteam kan gebruiken om problemen op te lossen. De volgende informatie wordt gekopieerd:

**Algemeen**

* `JourneyVersion UID` - Unieke id van deze versie van de rit
* `OrgID` - Id van uw organisatie (IMS)
* `orgName` - Naam van uw organisatie
* `sandboxName` - Naam van de zandbak waar de reis loopt
* `lastDeployedBy` - Gebruiker die de reis het laatst heeft gepubliceerd
* `lastDeployedAt` - Datum en tijdstip van de laatste publicatie


**Pauze en hervat** (inbegrepen wanneer de reis minstens eens is gepauzeerd)

* `lastPausedAt` - Datum en tijdstip van de laatste keer dat de reis werd gepauzeerd
* `lastPausedBy` - Naam weergeven van de gebruiker die de laatste pauze heeft uitgevoerd
* `lastPausedById` - Interne id van de gebruiker die de laatste pauze heeft uitgevoerd
* `lastResumedAt` - Datum en tijdstip van de laatste keer dat de reis werd hervat
* `lastResumedBy` - Geef de naam weer van de gebruiker die het laatste cv heeft uitgevoerd
* `lastResumedById` - Interne id van de gebruiker die het laatste cv heeft uitgevoerd

**Gepauzeerde reismontages** (in `pausedJourneySettings`, wanneer de reis wordt of is gepauzeerd)

* `pauseBehavior` - Wat gebeurt er met profielen tijdens de rit als deze worden gepauzeerd (bijvoorbeeld als u deze weggooit of op de juiste plaats houdt)
* `maxPauseDurationInMinutes` - Maximale pauzeduur in minuten, waarna de reis automatisch wordt hervat (bijvoorbeeld 20160 = 14 dagen)
* `transitionStateForAutoResume` - Staat die wordt toegepast wanneer de reis automatisch wordt hervat aan het eind van de pauzeperiode (bijvoorbeeld, stop of verdergaat)
* `pauseId` - Unieke id voor de huidige pauze-instantie
...
De tijdzone wordt gedefinieerd op het niveau van de reis. U kunt een vaste tijdzone invoeren of [!DNL Adobe Experience Platform] -profielen gebruiken om de tijdzone van de reis te definiëren. Als een tijdzone is gedefinieerd in [!DNL Adobe Experience Platform] -profiel, kan deze tijdens de rit worden opgehaald.
...
De montages van de onderbreking bepalen hoe lang een reis op activiteitenuitvoering wacht en hoe lang de profielen in een reis kunnen blijven.
...
>[!NOTE]
>
>De exacte definitie van wanneer een reis als &quot;voltooid&quot;wordt beschouwd varieert per vervoerstype. [&#x200B; zie gedetailleerde criteria &#x200B;](end-journey.md#journey-finished-definition).
...
Vanaf de release van [!DNL Adobe Journey Optimizer] juni 2024 is de wereldwijde time-out voor de reis van 30 naar 91 dagen gestegen. De gevolgen worden vermeld in de FAQ hieronder:
...
<table style="table-layout:auto">
  ...
  <tr style="border: 1;">
    <td>
      <p>Zal mijn altijd-op terugkomende Reader reis na 91 dagen stoppen?</p>
    </td>
    <td>
      <p>Nee. Een terugkomende Gelezen reis van het Publiek zonder einddatum blijft <strong> Levend </strong> zolang het wordt gepubliceerd. Het beweegt zich aan <strong> Voltooide </strong> status slechts 91 dagen na de uitvoering van zijn <strong> laatste voorkomen </strong>. De wereldwijde time-out van 91 dagen geldt voor individuele profielen die door de reis lopen (maximale actieve duur per profiel), niet voor de status Live van de reis.</p>
    </td>
  </tr>
  <tr style="border: 1;">
    <td>
      <p>Wat is het verschil tussen de reistijd van 91 dagen en het rapportagevenster van 91 dagen?</p>
    </td>
    <td>
      <p>Dit zijn twee verschillende concepten. De <strong> reis globale onderbreking </strong> (91 dagen) is de maximumtijd een individueel profiel binnen een reis actief kan blijven - na 91 dagen, wordt het profiel verlaten en zijn gegevens geschrapt. Het <strong> rapporterende venster </strong> (ongeveer 91 dagen) is een vertoningsgrens in UI: de prestatiesgegevens ouder dan ~91 dagen zijn niet meer zichtbaar in rapportering, maar de reis zelf blijft lopen en de nieuwe profielen blijven ingaan.</p>
    </td>
  </tr>
</table>

## Samenvoegbeleid {#merge-policies}

>[!CONTEXTUALHELP]
>id="ajo_journey_properties_merge_policy"
>title="Samenvoegbeleid"
>abstract="Het samenvoegbeleid wordt automatisch opgehaald op basis van de geselecteerde gebeurtenis of het geselecteerde publiek. Dit fusiebeleid wordt gebruikt in door volledige reis."

[!DNL Adobe Journey Optimizer] gebruikt samenvoegingsbeleid terwijl het terugwinnen van profielgegevens van [!DNL Adobe Experience Platform]. Afhankelijk van het type van reis, worden de verschillende samenvoegingsbeleidsvormen gebruikt:

* In **[leest publiek](read-audience.md)** of **[de kwalificatiereizen van het publiek](audience-qualification-events.md)**: het fusiebeleid van het publiek wordt gebruikt
* In **[de gebeurtenisreizen van de Eenheid](../event/about-events.md)**: het standaardfusiebeleid wordt gebruikt
* In **[Zaken gebeurtenis](../event/about-creating-business.md)** reizen: het fusiebeleid van het gerichte publiek in de volgende Gelezen publieksactiviteit wordt gebruikt

[!DNL Adobe Journey Optimizer] past het samenvoegbeleid toe dat gedurende de hele rit wordt gebruikt. Daarom als de veelvoudige publiek in een reis (bijvoorbeeld het gebruiken van binnen [`inAudience` functies &#x200B;](functions/functioninaudience.md)) wordt gebruikt, leidt dit tot inconsistenties met het fusiebeleid dat door de reis wordt gebruikt, wordt een fout opgeheven en de publicatie wordt geblokkeerd. Nochtans, als een inconsistent publiek in berichtverpersoonlijking wordt gebruikt, wordt een alarm niet opgeheven, ondanks de inconsistentie. Om deze reden, wordt het hoogst geadviseerd om het samenvoegbeleid te controleren verbonden aan uw publiek, wanneer dit publiek in berichtverpersoonlijking wordt gebruikt.

Meer over samenvoegingsbeleid leren, verwijs naar [[!DNL Adobe Experience Platform]  documentatie &#x200B;](https://experienceleague.adobe.com/en/docs/experience-platform/profile/merge-policies/overview){target="_blank"}.
...
De sectie **[!UICONTROL Schedule]** is alleen beschikbaar wanneer een **[!UICONTROL Read Audience]** -activiteit op het canvas is neergezet. Hiermee kunt u een specifieke datum/tijd en frequentie voor de rit definiëren. [&#x200B; Leer hoe te om een Lezen-publiek reis &#x200B;](read-audience.md#schedule) te plannen

>[!TIP]
>
>Wanneer het plannen van de reis, kunt u golf ook vormen die verzendt om reisacties in partijen in tijd te leveren. [&#x200B; leer hoe te om het gebruiken golven in reizen te verzenden &#x200B;](send-using-waves.md)
