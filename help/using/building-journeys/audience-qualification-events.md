...
>title="kwalificatie publiek"
>abstract="De reis van de trekker of voortzetting wanneer een profiel voor kwalificeert of een [!DNL Adobe Experience Platform] publiek weggaat. Aanbevolen voor het stromen publiek; gebruik een Gelezen activiteit van het Publiek voor partijscenario&#39;s."
...
>title="Label"
>abstract="Voeg een optioneel label toe om deze activiteit te identificeren in de logboeken van de rapportage- en testmodus."

>[!CONTEXTUALHELP]
>id="ajo_journey_event_segment_qualification_audience"
>title="Doelgroep"
>abstract="Selecteer het publiek van [!DNL Adobe Experience Platform] aan hefboomwerking. De reis luistert naar profielingangen en verlaat uit dit publiek."

>[!CONTEXTUALHELP]
>id="ajo_journey_event_segment_qualification_behavior"
>title="Gedrag"
>abstract="Kies of u naar de toegang van het publiek, naar het afsluiten of naar beide wilt luisteren."

>[!CONTEXTUALHELP]
>id="ajo_journey_event_segment_qualification_identity"
>title="Identiteitstype"
>abstract="Selecteer het identiteitstype dat wordt gebruikt om individuen te identificeren. Alleen naamruimten op basis van personen zijn beschikbaar."

>[!CONTEXTUALHELP]
>id="ajo_journey_event_segment_qualification_merge_policy"
>title="Samenvoegbeleid"
>abstract="Het samenvoegbeleid wordt automatisch opgehaald uit het geselecteerde publiek en wordt gedurende de hele reis toegepast."
>additional-url="https://experienceleague.adobe.com/en/docs/journey-optimizer/using/orchestrate-journeys/create-journey/journey-properties#merge-policies" text="Meer informatie over samenvoegingsbeleid"
...
Zie [ Voorwaarden ](../building-journeys/conditions.md#about_condition).
...
### Tijdstip waarop updates voor segmentlidmaatschap worden uitgevoerd {#timing-segment-membership}

Wanneer het gebruiken van partijmomentopnamen in een reis, kunnen om het even welke nieuwe segmentlidmaatschap slechts in verdere momentopnamen worden weerspiegeld. Als directe of segment-toevoegingen van de zelfde dag essentieel zijn, overweeg het stromen segmentatie of het verifiëren dat de segmentupdates door de volgende momentopname worden gevangen.
...
>[!NOTE]
>
>De spreidingstijd voor het stromen segmentlidmaatschap hangt van af hoe het lidmaatschap wordt geëvalueerd en waar het in de reis wordt gebruikt:
>
>* **knoop van de Kwalificatie van het publiek + het stromen segment:** wanneer een profiel voor een het stromen segment bij Edge kwalificeert, wordt dat lidmaatschap geprojecteerd van Edge aan Hub alvorens de reis op het kan handelen. Deze Edge-aan-Hub propagatie neemt typisch **15 tot 30 minuten**. Als de profielen niet een reis van de Kwalificatie van het Publiek zoals verwacht ingaan, sta voor dit propagatievenster (door een wachttijdactiviteit toe te voegen indien nodig) alvorens verder te onderzoeken. Voor gebruiksgevallen die echte real-time invoer vereisen, kunt u in plaats daarvan een trigger voor een eenheidsgebeurtenis overwegen.

#### Waarom niet alle gekwalificeerde profielen de reis kunnen betreden {#streaming-entry-caveats}

Wanneer het gebruiken van het stromen publiek met de **activiteit van de Kwalificatie van het publiek 0}, niet zullen alle profielen die voor het publiek kwalificeren noodzakelijk de reis ingaan.** Dit gedrag kan om de volgende redenen optreden:

* **Profielen reeds in het publiek**: Slechts zullen de profielen die onlangs voor het publiek kwalificeren nadat de reis wordt gepubliceerd ingang teweegbrengen. Profielen die al in het publiek aanwezig waren voordat ze werden gepubliceerd, worden niet ingevoerd. Op dezelfde manier wanneer een het stromen segment a **op tijd-gebaseerde voorwaarde** (bijvoorbeeld, &quot;gebeurtenis in de volgende 8 uren&quot;) gebruikt, zijn de profielen die reeds aan die voorwaarde voldeden alvorens het segment werd gecreeerd niet retroactief **geëvalueerd** — slechts profielen de waarvan gegevensveranderingen na segmentactivering worden beoordeeld tegen de voorwaarde.
