...
>[!CAUTION]
>
>U kunt een `dateTimeOnly` -expressie benutten of een functie gebruiken om om te zetten in een `dateTimeOnly` . Bijvoorbeeld: `toDateTimeOnly(@event{Event.offerOpened.activity.endTime})`, het gebied in de gebeurtenis die van vorm 2023-08-12T09 :46: 06Z is. De **tijdzone** wordt verwacht in de eigenschappen van uw reis, zodat is het niet mogelijk van UI om bij volledige tijdstempel te richten ISO-8601 die tijd en tijdzone zoals 2023-08-12T09 :46: 06.982-05 mengen. [Meer info](../building-journeys/timezone-management.md).
>
>Wanneer u een aangepaste wachtuitdrukking maakt met `toDateTimeOnly()`, moet u geen &#39;Z&#39; of verschuiving van tijdzones (bijvoorbeeld &#39;-05 :00&#39;) aan het resultaat toevoegen. De expressie moet een geldige ISO-datum-/tijdnotatie gebruiken die verwijst naar de geconfigureerde tijdzone van het transport zonder expliciete tijdzoneaanduidingen. Anders kunnen profielen blijven steken in de wachttijdactiviteit.
>
>**Correct voorbeeld:** `toDateTimeOnly(concat(toString(toDateOnly(nowWithDelta(2, "days"))),"T10:00:00"))`
>
>**Onjuist voorbeeld:** `toDateTimeOnly(concat(toString(toDateOnly(nowWithDelta(2, "days"))),"T10:00:00Z"))` ❌ (bevat &quot;Z&quot;)
