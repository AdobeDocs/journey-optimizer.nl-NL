...
>[!CAUTION]
>
>Houd rekening met het volgende wanneer u werkt met `dateTimeOnly` -expressies:
>
>* U kunt een `dateTimeOnly` -expressie rechtstreeks gebruiken of er een functie voor gebruiken, bijvoorbeeld: `toDateTimeOnly(@event{Event.offerOpened.activity.endTime})` waarbij de veldwaarde in het formulier staat `2023-08-12T09:46:06Z` .
>* De **tijdzone** wordt bepaald in de reiseigenschappen. Het is daarom niet mogelijk om vanuit de gebruikersinterface te verwijzen naar een volledig ISO-8601-tijdstempel dat verschuiving van tijd en tijdzone combineert, zoals `2023-08-12T09:46:06.982-05` . [Meer informatie](../building-journeys/timezone-management.md)
>* Wanneer het bouwen van een douane wacht uitdrukking met `toDateTimeOnly()`, voeg **niet** toe `Z` of een tijdzonecompensatie (b.v., `-05:00`). De expressie moet verwijzen naar de geconfigureerde tijdzone van de reis zonder expliciete tijdzoneaanduidingen. Anders kunnen profielen vastzitten in de wachttijdactiviteit.
>
>| | Voorbeeld |
>|---|---|
>| **Juist** | `toDateTimeOnly(concat(toString(toDateOnly(nowWithDelta(2, "days"))),"T10:00:00"))` |
>| **Onjuist** | `toDateTimeOnly(concat(toString(toDateOnly(nowWithDelta(2, "days"))),"T10:00:00Z"))` ❌ (contains `Z`) |

Om te bevestigen dat de wachttijdactiviteit zoals verwacht werkt, kunt u step gebeurtenissen gebruiken. [Meer info](../reports/query-examples.md#common-queries).
