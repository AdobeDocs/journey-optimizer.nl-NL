...
In dit geval worden alle stappen beschreven die zijn vereist om een e-mail te verzenden via de integratie met [!DNL Adobe Campaign] v7 en [!DNL Adobe Campaign] v8.
...
1. Onder **Beleid** > **Platform** > **Opsommingen**, selecteer het **type van Gebeurtenis** (eventType) opsomming. Maak een nieuw gebeurtenistype (&quot;reis-gebeurtenis&quot;, in ons voorbeeld). Gebruik de interne naam van het gebeurtenistype wanneer u het JSON-bestand later schrijft.
   ...
   ![ vorm een gebeurtenis in [!DNL Adobe Journey Optimizer] met schema en gebiedsselectie ](assets/accintegration-uc-1.png)
...
1. Ontwerp uw sjabloon. In dit voorbeeld wordt personalisatie toegepast op de voornaam en het ordernummer van het profiel. De voornaam bevindt zich in de gegevensbron van [!DNL Adobe Experience Platform] en het ordernummer is een veld van de Journey Optimizer-gebeurtenis. Zorg ervoor dat u de juiste veldnamen gebruikt in Campagne.
...
1. Maak een gebeurtenis. Neem het veld &quot;purchaseOrderNumber&quot; op.
   ...
   ![ het scherm van de actieconfiguratie van de Douane voor [!DNL Adobe Campaign] Klassieke integratie ](assets/accintegration-uc-5.png)

1. Maak een actie in Journey Optimizer die overeenkomt met uw campagnemplate. In het **type van Actie** drop-down, uitgezochte **[!DNL Adobe Campaign]Klassiek**.

   ![ het type van Actie selectie die [!DNL Adobe Campaign] Klassieke optie ](assets/accintegration-uc-6.png) toont
