...
Gebruik de activiteit van het publiek lezen om reizen met bepaald publiek te beginnen. U kiest het publiek en wanneer het loopt; dan gebruik [ voorwaarden ](#audience-targeting-in-journeys), tijdopnemers, en acties om de weg van elk profiel te personaliseren.
...
**Zeer belangrijke grenzen:** Één Gelezen Publiek per reis (moet de eerste activiteit zijn); één publiek per activiteit; tot vijf gelijktijdige Gelezen looppas van de Publiek per organisatie; 20.000 profielen per seconde per zandbak; de baanonderbreking van 12 uur. Volledige details in [ Grafieken en beperkingen ](../start/guardrails.md#read-segment-g).

**Eerste vereisten:** Een [!DNL Adobe Experience Platform] publiek dat wordt gebouwd en geëvalueerd (Realized status), een op mensen-gebaseerde identiteit namespace die voor de reis wordt geselecteerd, en-voor het terugkomen looppas-begrip van [ het plannen en productiegrenzen ](../start/guardrails.md#read-segment-g).

Bijvoorbeeld, kan het `Luma app opening and checkout` publiek dat in [ wordt gecreeerd het 2} gebruiksgeval van de Bouwstijl van publiek {als ingangspunt worden gebruikt. ](../audience/about-audiences.md) Alle gekwalificeerde profielen voeren de reis en de vooruitgang door geïndividualiseerde wegen in gebruikend voorwaarden, tijdopnemers, gebeurtenissen, en acties.
...
>title="Naamruimte"
>abstract="Kies welke identiteit (bijvoorbeeld e-mail, ECID) wordt gebruikt om personen die de reis binnenkomen te identificeren. Het veld wordt standaard voorgevuld met de laatst gebruikte naamruimte."
...
Alle guardrails en beperkingen voor de **Gelezen activiteit van het Publiek** (gelijktijdig, productie, één publiek per activiteit, baanonderbreking, pogingen, en meer) worden vermeld in [ Grafieken en beperkingen ](../start/guardrails.md#read-segment-g).

**Aanbevelingen**

* Als beste praktijken, gebruik partijpubliek in a **Gelezen publiek** activiteit voor betrouwbare en verenigbare tellingen. Lees het publiek wordt ontworpen voor partijgebruik gevallen. Als uw gebruiksgeval gegevens in real time vereist, gebruik in plaats daarvan de [ kwalificatie van het publiek ](audience-qualification-events.md) activiteit.
...
* Voor informatie over de timing van de publieksmomentopname, de vensters van de partijsegmenteringsvoltooiing, en hoe te om uw reis te verzekeren altijd looppas op de hoogste gegevens, zie [ Timing en gegevenspropagatie ](#timing-and-data-propagation). Voor terugkerende reizen kunt u overwegen de optie **[!UICONTROL Trigger after batch audience evaluation]** in te schakelen om de uitvoering automatisch uit te stellen totdat de meest recente publieksmomentopname gereed is. [Meer info](#schedule).
...
>[!NOTE]
>
>**de status van de Reis en globale onderbreking van 91 dagen:**
>
>* **one-shot** Leest publiekstrajecten beweging aan de **Voltooide** status 91 dagen ([ reis globale onderbreking ](journey-properties.md#global_timeout)) na de reisuitvoering.
>* **het Terugkomen** leest publiekstrajecten zonder einddatum **Levend** blijven zolang de reis wordt gepubliceerd. Zij bewegen zich aan **Voltooide** status 91 dagen na de uitvoering van hun **laatste voorkomen**.
>* De onderbreking van 91 dagen is op individuele **profielen** van toepassing die door de reis (de maximumtijd een profiel kan actief blijven) stromen, niet aan de Levende status van de reis.
>* Het 91 dag **meldend venster** is een afzonderlijk concept: UI toont prestatiesgegevens voor ongeveer de laatste 91 dagen. Oudere gegevens zijn niet toegankelijk in UI maar de reis blijft lopen. [Meer informatie](journey-properties.md#global_timeout)
...
## Doelgerichtheid van het publiek bij reizen {#audience-targeting-in-journeys}
...
* **`inAudience()`voorwaarde timing:** wanneer het gebruiken van `inAudience()` in een voorwaardenknoop binnen een Reis van de Audience van het Lezen, wordt het segmentlidmaatschap gelezen van de partijprojectie van het profiel. De gegevens in deze projectie worden verfrist binnen **2 uren** na opname. Voor volledige details op scenario&#39;s van de propagatietiming, verwijs naar de [ documentatie van de functie InAudience ](functions/functioninaudience.md#propagation-timing).
...
Voor de volledige lijst van Gelezen Garanties van de Publiek (met inbegrip van retry en productiegrenzen), zie [ Grafieken en beperkingen ](../start/guardrails.md#read-segment-g).
...
* [Afvoerkanalen en beperkingen](../start/guardrails.md#read-segment-g)
