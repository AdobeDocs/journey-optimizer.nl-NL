...
### De status van de push-optie controleren {#push-opt-out-status}

Push-out voor mobiele apps wordt op apparaatniveau afgehandeld: wanneer een gebruiker meldingen op zijn apparaat uitschakelt, wordt het pushtoken uit het profiel verwijderd. De **aanwezigheid van een duw.teken** op een profiel is daarom de indicator van impliciete duw toestemming.

Om de status van de pushtoestemming van een profiel in Adobe Experience Platform te verifiëren:

1. Open het profiel in de sectie **[!UICONTROL Profiles]** van Adobe Experience Platform.
1. Ga naar de tab **[!UICONTROL Attributes]** en zoek de veldgroep **[!UICONTROL Push Notification Details]** .
1. Als er een pushtoken aanwezig is, heeft het profiel impliciet ingestemd met het ontvangen van pushberichten. Als er geen token wordt gevonden, heeft de gebruiker de functie uitgeschakeld op apparaatniveau.

>[!NOTE]
>
>Voor nalevingsgebruiksgevallen die expliciete het volgen van de duwtoestemming vereisen, gebruik het **`consents.marketing.push.val`** attribuut van de [&#x200B; Consents en het gebiedsgroep van de Voorkeur &#x200B;](https://experienceleague.adobe.com/docs/experience-platform/xdm/field-groups/profile/consents.html?lang=nl-NL){target="_blank"}. De waarde `y` geeft de expliciete opt-in aan. `n` geeft de expliciete opt-out aan.
