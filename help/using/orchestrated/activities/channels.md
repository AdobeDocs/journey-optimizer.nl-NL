...
>title="Target"
>abstract="Tijdelijke aanduiding voor doelsectie"
...
Met [!DNL Adobe Journey Optimizer] kunt u campagnes automatiseren en uitvoeren via verschillende kanalen (e-mail, SMS, pushmeldingen en direct mail) voor zowel marketing- als transactieberichten. U kunt deze kanaalactiviteiten combineren tot het campagnecanvas om via meerdere kanalen geordende campagnes te maken. Deze campagnes kunnen acties teweegbrengen die op klantengedrag en gegevens worden gebaseerd.

Bijvoorbeeld:

* Verzend een welkomstreeks via e-mail, SMS, push en direct mail.
...
>[!CAUTION]
>
>Alleen SMS-, push-, e-mail- en direct-mailkanalen worden ondersteund in geordende campagnes.
...
>title="Categorie"
>abstract="Kies Marketing of Transactie voor deze kanaalactiviteit. De berichten van de marketing gebruiken marketing kanaalconfiguraties en volgen uw standaard bedrijfsregels. Transactieberichten zijn bedoeld voor operationele communicatie — die vaak wordt geactiveerd door een handeling van een individu (bijvoorbeeld het opnieuw instellen van een wachtwoord of het aanschaffen van een bevestiging) of voor tijdgevoelige berichten zoals onderbrekingen of annuleringen. Zij gebruiken transactiekanaalconfiguraties, bedrijfsregels worden overgeslagen, en opt-in wordt niet vereist."
...
1. Voeg een kanaalactiviteit toe aan het canvas. De beschikbare kanaalactiviteiten zijn **[!UICONTROL Email]** , **[!UICONTROL SMS]** , **[!UICONTROL Push]** en **[!UICONTROL Direct mail]** .

   ![ beeld dat het canvas met beschikbare activiteiten toont ](../assets/channel-add.png)

1. Gebruik in de rechtertrack het veld **[!UICONTROL Category]** om **[!UICONTROL Marketing]** of **[!UICONTROL Transactional]** voor dit bericht te kiezen. Transactieberichten vereisen geen opt-in en zijn geschikt voor tijdgevoelige communicatie zoals onderbrekingen, noodsituaties of annuleringen.

1. Selecteer de activiteit en klik op **[!UICONTROL Edit email]** , **[!UICONTROL Edit SMS]** , **[!UICONTROL Edit Push]** of **[!UICONTROL Edit direct mail]** afhankelijk van het gekozen kanaal.
...
1. **pas het begrenzen van regels** toe
   ...
   Selecteer in de vervolgkeuzelijst **[!UICONTROL Rule set]** een kanaalregel die is ingesteld om de bijschilderingsregels toe te passen op uw campagne. De reeksen van de kanaalregel van hefboomwerking staan u toe om frequentie het begrenzen door communicatie type te plaatsen om het overbelasten van klanten met gelijkaardige berichten te verhinderen. [ Leer hoe te met regelreeksen ](../../conflict-prioritization/rule-sets.md) te werken.
...
+++**Overeenkomst van het Spoor** (E-mail en SMS).
...
<td><a href="../../push/create-push.md"><img alt="duwen" src="../../channels/assets/do-not-localize/push.png"></a><a href="../../push/create-push.md"><strong>Een pushmelding maken</strong></a></td><td><a href="../../direct-mail/create-direct-mail.md"><img alt="direct mail" src="../../channels/assets/do-not-localize/direct-mail.jpg"></a><a href="../../direct-mail/create-direct-mail.md"><strong>Een directe e-mail maken</strong></a></td>
</tr></table>

### Aanpassing toevoegen

Personalization in geordende campagnes werkt op dezelfde manier als andere [!DNL Journey Optimizer] -campagnes of -reizen, met enkele belangrijke verschillen die specifiek zijn voor het georkestreerde canvas.
...
### Uw inhoud controleren en testen {#simulate-content-test-profiles}
...
Wanneer u inhoud met **testprofielen** in een Geordende campagne simuleert, zijn twee belangrijke beperkingen van toepassing:

* **Uitvoering moet de kanaalactiviteit in test** bereikt hebben - stel de campagne in test in werking gebruikend de **[!UICONTROL Start]** knoop zodat bereikt het werkschema de kanaalactiviteit u wilt simuleren. In de testmodus wordt de kanaalactiviteit gepauzeerd, zodat een kanaalactiviteit die optreedt nadat een andere kanaalactiviteit nooit is bereikt. U kunt **[!UICONTROL Simulate Content]** niet gebruiken voor die downstream-kanaalactiviteiten. Zie [ uw campagne testen alvorens ](../start-monitor-campaigns.md#test) te publiceren.

* **het testprofiel moet het doel van de kanaalactiviteit** aanpassen - gebruik een testprofiel dat tot het publiek behoort dat door die kanaalactiviteit wordt gericht. Als het profiel niet in dat publiek aanwezig is, wordt door het selecteren geen voorvertoning van de inhoud weergegeven. Zie [ Uitgezochte testprofielen ](../../content-management/test-profiles.md).

## Bericht verzenden bevestigen

Door gebrek, voor terugkerende georkestreerde campagnes, wordt de berichtlevering gepauzeerd tot u uitdrukkelijk toestuur goedkeurt. Na het publiceren van de campagne, bevestig het verzendverzoek van de de eigenschappen van de kanaalactiviteit ruit.

![ beeld dat de Confirm knoop ](../assets/confirm-sending.png) toont

Het verzenden van een bevestiging kan worden uitgeschakeld voordat de georkestreerde campagne wordt gepubliceerd. Selecteer hiertoe de kanaalactiviteit op het canvas om de eigenschappen weer te geven en schakel **[!UICONTROL Send without confirmation]** in.

![ beeld dat Send zonder bevestigingsknoop toont ](../assets/send-without-confirmation.png)

## Snelheidbeheersing instellen {#rate-control}

[!DNL Journey Optimizer] staat u toe om tariefcontrole voor uitgaande acties in Geordende campagnes toe te laten.
...
Ga als volgt te werk om de snelheidscontrole in te stellen:

1. Selecteer een uitgaande kanaalactiviteit op het canvas en klik op **[!UICONTROL Edit email]** , **[!UICONTROL Edit SMS]** of **[!UICONTROL Edit Push]** afhankelijk van het gekozen kanaal.

   ![ beeld dat het canvas met een e-mailactiviteit toont ](../assets/channel-edit.png)

1. Navigeer naar de tab **[!UICONTROL Schedule]** en schakel de optie **[!UICONTROL Throttle delivery]** in de sectie **[!UICONTROL Delivery settings]** in.

   ![ de controlemontages van het Tarief met de optie van de throttle levering en leveringstarief per seconde ](../assets/rate-control.png)

1. Geef de gewenste **[!UICONTROL Delivery rate]** per seconde op.
...
>[!IMPORTANT]
>
>Wanneer het plaatsen van een leveringstarief, is het maximumtijdskader waarvoor een campagnepubliek kan uitvoeren 12 uren. Als het leveringspercentage is ingesteld op een waarde die het niet mogelijk maakt dat alle gebruikers het bericht in de periode van 12 uur ontvangen, worden de resterende profielen uitgesloten van de campagne. U kunt de telling van deze uitgesloten profielen in het campagnerapport zien.

## Volgende stappen {#next}

Wanneer de berichtinhoud klaar is, navigeert u met de pijl **[!UICONTROL Back]** terug naar uw geordende campagne. Vervolgens kunt u de activiteiten op het canvas ordenen en de campagne publiceren om berichten te verzenden. [ leren hoe te om Geordende campagnes te beginnen en te controleren ](../start-monitor-campaigns.md)
