...
| `xdm:responseFormat.xdm:includeMetadata` | Een object dat wordt gebruikt om op te geven welke aanvullende metagegevens worden geretourneerd. Als deze eigenschap niet is opgenomen, worden `xdm:id` en `repo:etag` standaard geretourneerd. | `name` |
| `xdm:responseFormat.xdm:activity` | Deze markering identificeert de specifieke meta-gegevensinformatie die voor `xdm:activity` wordt teruggekeerd. | `name` |
| `xdm:responseFormat.xdm:option` | Deze markering identificeert de specifieke meta-gegevensinformatie die voor `xdm:option` wordt teruggekeerd. | `name`, `characteristics` |
| `xdm:responseFormat.xdm:placement` | Deze markering identificeert de specifieke meta-gegevensinformatie die voor `xdm:placement` wordt teruggekeerd. | `name`, `channel`, `componentType` |

**Antwoord**

Een succesvolle reactie keert informatie over uw voorstel, met inbegrip van zijn uniek `xdm:propositionId` terug.
...
| `xdm:propositions` | Dit object bevat één beslissingsvoorstel. Er kunnen meerdere opties worden geretourneerd voor de beslissing. Als er geen opties worden gevonden, wordt het fallback-aanbod van het besluit geretourneerd. Enige besluitvoorstellingen omvatten altijd of een `options` bezit of een `fallback` bezit. Indien aanwezig, kan de `options` bezit niet leeg zijn. |
...
| `xdm:propositions.xdm:fallback.dc:format` | De fysieke of digitale manifestatie van de bron. De indeling moet meestal het mediatype van de bron bevatten. Het formaat kan worden gebruikt om te bepalen welke software, hardware of andere apparatuur nodig is om de bron weer te geven of te gebruiken. Het wordt aanbevolen een waarde te selecteren in een gecontroleerde woordenlijst, bijvoorbeeld de lijst met [Internet Media Types](http://www.iana.org/assignments/media-types/) die computermedia-indelingen definiëren. | `"dc:format": "image/png"` of `"image/jpeg"` |
