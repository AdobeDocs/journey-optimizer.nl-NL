...
1. Voor op code-gebaseerde ervaringsreizen en campagnes, moeten de interactiegebeurtenissen manueel worden verzonden om erop te wijzen wanneer een gebruiker met de inhoud heeft gecommuniceerd. Dit gebeurt via de opdracht `sendEvent` .
   ...
   >[!IMPORTANT]
   >
   >Het `tokens` -veld in `propositionAction` is van essentieel belang voor nauwkeurige tracking en toewijzing in Adobe Journey Optimizer-beslissingen. Deze tokens maken het volgende mogelijk:
   >* Correcte kliktoewijzing voor beslissingsactiviteiten
   >* Nauwkeurige rapportage van gebruikersinteracties met beslissingsinhoud
   >* Optimalisatie van de prestaties van de aanbieding op basis van de betrokkenheid van de gebruiker
   >
   >De tokens worden meestal gevonden in `proposition.items[0].characteristics.tokens` en moeten altijd worden opgenomen wanneer gebruikers interactie met de beslissingsinhoud volgen.
