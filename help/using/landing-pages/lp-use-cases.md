...
1. Publiceer en voer de reis uit. [Meer info](../building-journeys/journey.md).
...
## Levering van gebeurtenis voor indienen landingspagina {#leverage-lp-event}

U kunt informatie gebruiken die op een landingspagina is verzonden om verdere acties uit te voeren. Als een gebruiker zich bijvoorbeeld abonneert op een bepaalde abonnementenlijst, kunt u deze gegevens gebruiken om een e-mail te verzenden waarin andere abonnementslijsten worden aanbevolen.

Om dit te doen, moet u a [ regel-gebaseerde eenheidsgebeurtenis tot stand brengen ](../event/about-creating.md) die op **[!UICONTROL AJO Email Tracking Experience Event Schema]** wordt gebaseerd die de voorleggingsinformatie bevatten en [ deze gebeurtenis in een reis ](../building-journeys/general-events.md) gebruiken.

>[!NOTE]
>
>Houd er bij het werken met verzendgebeurtenissen voor de landingspagina rekening mee dat het veld event `interactionType` niet altijd exact de specifieke actie van de gebruiker weerspiegelt. Om nauwkeurig te bepalen of een gebruiker heeft gekozen uit, geabonneerd, of een andere actie uitgevoerd, verifieer altijd de daadwerkelijke profielattributen (zoals toestemmingsvoorkeur) of de waarden van de vormgebieden eerder dan het baseren zich alleen op de gebeurtenis `interactionType`.
