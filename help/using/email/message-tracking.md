...
### Houd meerdere e-mails bij {#track-across-multiple-emails}

Een uniek volgend herkenningsteken (urlID) wordt slechts geproduceerd wanneer zowel **URL** als het **etiket** uniek zijn. Koppelingen met dezelfde URL en hetzelfde effectieve label (inclusief wanneer het label leeg is) gebruiken dezelfde URLID opnieuw, wat betekent dat u niet kunt zien op welke koppeling is geklikt.

Als u dezelfde URL in meerdere e-mails (of meerdere keren per e-mail) wilt bijhouden, gebruikt u een uniek label voor elke vergelijkbare URL. Als u dat niet doet, kan [!DNL Journey Optimizer] niet bijhouden op welke koppeling is geklikt. U kunt verschillende labels instellen in de Designer-e-mail of, voor HTML, via het kenmerk `data-label` .

| URL | Tag | Label | urlID, gedrag |
|-----|-----|-------|----------------|
| www.example.com | Eerste | (leeg) | Hiermee wordt een urlID opgehaald (bijvoorbeeld A) |
| www.example.com | Seconde | (leeg) | Gebruikt urlID A opnieuw — kan niet zien op welke koppeling is geklikt |
| www.example.com | Derde | Eerste label | Hiermee wordt een urlID opgehaald (bijvoorbeeld B) |
| www.example.com | Vierde | Tweede label | Hiermee wordt een urlID opgehaald (bijvoorbeeld C) |
...
1. Kies het type koppeling dat u wilt maken:
   ...
   * Selecteer **[!UICONTROL Landing page]** om een koppeling naar een bestemmingspagina in te voegen. [Meer informatie](../landing-pages/create-lp.md)

   * Selecteer **[!UICONTROL One click Opt-out]** om een koppeling in te voegen zodat gebruikers zich snel kunnen afmelden bij uw communicatie zonder dat ze hoeven te bevestigen dat ze het abonnement moeten opzeggen. [Meer info](email-opt-out.md#one-click-opt-out).
...
>[!CAUTION]
>
>Wanneer zowel het **etiket** als **URL** van een knoop editable in a [ aanpasbaar fragment ](../content-management/customizable-fragments.md) worden gemaakt, toont het volgen rapporten URL in plaats van het knoopetiket.
