...
Een uniek volgend herkenningsteken (urlID) wordt slechts geproduceerd wanneer zowel **URL** als het **etiket** uniek zijn. Koppelingen met dezelfde URL en hetzelfde effectieve label (inclusief wanneer het label leeg is) gebruiken dezelfde URLID opnieuw, wat betekent dat u niet kunt zien op welke koppeling is geklikt. Als u dezelfde URL in meerdere e-mails (of meerdere keren per e-mail) wilt bijhouden, gebruikt u een uniek label voor elke vergelijkbare URL. Als u dat niet doet, kan [!DNL Journey Optimizer] niet bijhouden op welke koppeling is geklikt. U kunt verschillende labels instellen in de Designer-e-mail of, voor HTML, via het kenmerk `data-label` .
