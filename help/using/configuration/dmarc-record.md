...
[!DNL Journey Optimizer] biedt nu rechtstreeks ondersteuning voor de DMARC-technologie in de beheerinterface, zodat u problemen met de te leveren items kunt voorkomen terwijl u controle verkrijgt over e-mail die niet wordt geverifieerd. [Meer informatie](#implement-dmarc)
...
* Wanneer **het delegeren van om het even welk nieuw subdomain** aan Adobe, kunt u **opstelling DMARC** direct in de [!DNL Journey Optimizer] beleidsinterface. [ leer hoe ](#set-up-dmarc)
...
1. Selecteer een subdomein waaraan geen DMARC-record is gekoppeld en vul de sectie **[!UICONTROL DMARC record]** in op basis van de behoeften van uw organisatie. De stappen om de het verslaggebieden van DMARC te bevolken zijn gedetailleerd in [ deze sectie ](#set-up-dmarc).

   <!--![](assets/dmarc-record-edit-full.png)-->

   >[!NOTE]
   >
   >Afhankelijk van of een DMARC-record wordt gevonden met het bovenliggende domein of niet, kunt u kiezen of u de waarden van het bovenliggende domein wilt gebruiken of dat Adobe de DMARC-record wil beheren. [Meer informatie](#manage-dmarc-with-adobe)
...
1. Naar keuze, voeg één of meerdere e-mailadressen van uw keus toe om erop te wijzen waar **DMARC** op e-mail meldt dat [ authentificatie ](#how-dmarc-works) binnen uw organisatie zou moeten ontbreken. U kunt maximaal vijf adressen voor elk rapport toevoegen.
   ...
   >* Deze hoogst technische rapporten verstrekken een overzicht van e-mails die voor de gek houden worden geprobeerd en het best door een derdehulpmiddel worden verstopt.
...
Bij het opzetten van een DMARC-record wordt een DNS TXT-record toegevoegd aan de DNS-instellingen van uw domein die uw DMARC-beleid opgeven.

**DNS propagatietiming**

DNS de veranderingen vergen tijd over Internet, typisch tussen een paar minuten aan 48 uren te verspreiden. Als u zojuist een DMARC-configuratiewijziging hebt aangebracht en probeert de update onmiddellijk te verifiëren, worden mogelijk fouten weergegeven of zijn de wijzigingen nog niet gedetecteerd.

Toestaan voldoende tijd voor de DNS verslagen om zich te verspreiden alvorens te proberen om uw opstelling van DMARC te verifiëren. Als u problemen na 48 uur blijft ervaren, verifieer dat de DNS verslagen correct aan uw het ontvangen oplossing werden toegevoegd.
