...
### Een schema maken {#create-schema}
...
1. Selecteer een schematype, bijvoorbeeld **[!UICONTROL Individual Profile]**, en klik **daarna**.
...
1. In de **sectie van de Groepen van het Gebied**, op de linkerzijde, voegt de klik **&#x200B;**&#x200B;toe en selecteert de aangewezen gebiedsgroepen. Voeg de veldgroep **[!UICONTROL Profile test details]** toe.
...
>[!IMPORTANT]
>
>Wanneer het creëren van of het vervangen van een dataset voor de opname van het testprofiel, zorg ervoor dat het schema de correcte identiteitsbeschrijver heeft die op het primaire identiteitsgebied (b.v., `/personID`) voor voorgenomen namespace wordt toegepast. Als de identiteitsbeschrijver mist of verkeerd gevormd, profielen die in deze dataset worden opgenomen kunnen niet als testprofielen (`testProfile = true`) worden gemarkeerd, zelfs als het innameproces met succes voltooit.
>
>Als de testprofielen na inname niet correct zijn gemarkeerd:
>
>1. Herzie het schema verbonden aan uw dataset.
>1. Controleer of het primaire identiteitsveld de juiste identiteitsbeschrijving voor de naamruimte heeft (zie de bovenstaande stappen 6-7).
>1. Als de descriptor ontbreekt, werkt u het schema bij om de identiteitsbeschrijving toe te voegen en de gegevens opnieuw in te voeren.

### Een gegevensset maken {#create-dataset}
...
## Testprofielen maken met een CSV-bestand {#create-test-profiles-csv}
...
1. Voeg één kolom toe voor elk vereist veld. Voeg het primaire identiteitsveld (`personID` in het bovenstaande voorbeeld) en het `testProfile` veld dat is ingesteld op `true` toe.
...
1. Kies **[!UICONTROL Map CSV to XDM schema]**, dan klik **Lancering**.
...
U moet een profielschema gebruiken dat de **[!UICONTROL Profile test details]** gebiedsgroep bevat. De markering `testProfile` maakt deel uit van deze veldgroep.
Wanneer u een profiel maakt, moet u de waarde `testProfile = true` doorgeven.

U kunt ook een bestaand profiel bijwerken en de markering `testProfile` ervan wijzigen in `true` .
